...
public class GestionPrestamo {
	...
	public boolean nuevoPrestamo(LocalDateTime fechaFin,Cliente cliente, Material material) {
		...
		while(i<this.prestamos.length && this.prestamos[i]!=null) {
			if(this.prestamos[i]!=null && this.prestamos[i].getCliente().getDni().equals(cliente.getDni()))
				totalPrestamos++;
			i++;
		}
		if(totalPrestamos<5) {
			...
			while(this.prestamos[i]!=null)
				i++;
			this.prestamos[i==0?0:i-1] = new Prestamo(fechaFin, cliente, material);
			return true;
		}
		return false;
	}
	public boolean devolverPrestamo(LocalDateTime fechaInicio, LocalDateTime fechaFin, Cliente cliente, Material material) {
		int i=0;
		//no se permiten más de 5 materiales prestados por cliente.
		//un material está prestado si no tiene fecha de fin de préstamo asignada
		while(i<this.prestamos.length && this.prestamos[i]!=null) {
